...
function ScoreCard({ label, value, icon, accent, net }) {
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: '0.68rem', textTransform: 'uppercase', letterSpacing: 1, color: '#8A9E8F', marginBottom: 4 }}>
          {label}
        </div>
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 'clamp(1rem, 3vw, 1.3rem)', color: net !== undefined ? (net >= 0 ? '#4CAF7D' : '#E07B54') : '#EDE8DC', lineHeight: 1.2 }}>
          {net !== undefined && (net >= 0 ? '+' : '-')}{fcfa(value)}
        </div>
      </div>
    </div>
  );
...
function EmptyState({ text }) {
  return (
    <div style={{ textAlign: 'center', padding: '32px 16px', color: '#8A9E8F', fontSize: '0.85rem' }}>
      <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#1E3328" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" style={{ marginBottom: 10, display: 'block', margin: '0 auto 10px' }}>
  ...
  return (
    <>
      <style>{`
        ...
        .period-select {
          background: #122019; color: #EDE8DC;
          border: 1px solid #1E3328; border-radius: 8px;
          padding: 7px 12px; font-size: 0.82rem; cursor: pointer;
          font-family: 'IBM Plex Sans', sans-serif;
        }
        .btn-refresh {
          background: #C9A84C; color: #0D1B14;
        ...
        .tx-row {
          ...
          padding: 11px 0; border-bottom: 1px solid #1E3328;
        }
        ...
        .tx-libelle { display: block; font-size: 0.83rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .tx-cat     { font-size: 0.7rem; color: #8A9E8F; }
        .tx-right   { text-align: right; flex-shrink: 0; }
        .tx-amount  { display: block; font-family: 'DM Mono', monospace; font-size: 0.82rem; }
        ...
        @media (max-width: 380px) {
          .period-select { font-size: 0.75rem; padding: 6px 8px; }
      ...
      <div className="topbar">
        ...
        <div className="topbar-right">
          <select className="period-select" value={period} onChange={e => setPeriod(Number(e.target.value))}>
      ...
      <div className="dash-content">
        ...
        <div className="score-grid">
          ...
          <ScoreCard label="Charges totales"    value={kpis.charges} accent="#E07B54" icon={<SVGSpend />} />
          <ScoreCard label="Résultat net"        value={Math.abs(kpis.net)} accent={kpis.net >= 0 ? '#C9A84C' : '#E07B54'} icon={<SVGTrend up={kpis.net >= 0} />} net={kpis.net} />
        </div>
            ...
            {dailySeries.length > 0 ? (
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={dailySeries} margin={{ top: 5, right: 5, bottom: 5, left: 0 }}>
                  ...
                  <XAxis dataKey="date" tick={{ fill: '#8A9E8F', fontSize: 9 }}/>
                  <YAxis tick={{ fill: '#8A9E8F', fontSize: 9 }} tickFormatter={v => `${(v/1000).toFixed(0)}k`} width={36}/>
                  <Tooltip content={<CustomAreaTooltip />}/>
                  <Legend wrapperStyle={{ fontSize: 11 }}/>
                  <Area type="monotone" dataKey="recettes" stroke="#4CAF7D" strokeWidth={2} fill="url(#gR)"/>
                  <Area type="monotone" dataKey="depenses" stroke="#E07B54" strokeWidth={2} fill="url(#gD)"/>
                </AreaChart>
              </ResponsiveContainer>
            ) : (
              <EmptyState text="Aucune donnée pour la période sélectionnée" />
            )}
          </div>
        ...
        <div className="bottom-row">
          ...
          <div className="chart-card">
            <div className="section-head">
              ...
              <a href="/dashboard/transactions" className="btn-secondary">Tout voir</a>
            </div>
            {(data?.transactions?.length > 0) ? (
              <div>
                {data.transactions.slice(0, 7).map((tx, i) => (
                  <div key={tx.id || i} className="tx-row">
                    ...
                    <div className="tx-right">
                      <span className="tx-amount" style={{ color: tx.type === 'RECETTE' ? '#4CAF7D' : '#E07B54' }}>
              ...
              <EmptyState text="Envoyez votre première opération sur WhatsApp" />
            )}
          </div>
          ...
          <div>
            ...
            <div className="ai-cta">
              ...
              <p style={{ fontSize: '0.82rem', color: '#8A9E8F', lineHeight: 1.6, marginBottom: 16 }}>
                Obtenez 3 conseils personnalisés basés sur vos transactions réelles.
              </p>
              <a href="/dashboard/advice" style={{
                display: 'inline-flex', alignItems: 'center', gap: 8,
                background: 'linear-gradient(135deg, #C9A84C, #A07820)',
                color: '#0D1B14', border: 'none', borderRadius: 8,
